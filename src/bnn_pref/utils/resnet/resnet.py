    ...
    def __call__(self, x, act, train=True):
        ...
        if self.downsample:
            residual = nn.Conv(
                ...
                kernel_init=self.kernel_init
                if self.param_dict is None
                else lambda *_: jnp.array(self.param_dict["downsample"]["conv"]["weight"]),
                use_bias=False,
                dtype=self.dtype,
            )(residual)
...
class ResNet(nn.Module):
    ...
    Attributes:
        output (str):
            Output of the module. Available options are:
                ...
                - 'embeddings': Output is a tensor of shape [N, embedding_dim]
        pretrained (str):
    ...
    def setup(self):
        ...
        if self.pretrained == "imagenet":
            ckpt_file = ckpt_utils.download(self.ckpt_dir, URLS[self.architecture])
            h5_file = h5py.File(ckpt_file, "r")
            # Convert h5py file handle to JAX arrays to avoid tracer leaks
            # Load all weights into memory as JAX arrays
            self.param_dict = self._load_h5py_to_jax(h5_file)
            h5_file.close()  # Close the file handle
            # print(f"Loaded {self.architecture} pretrained weights from {ckpt_file}")
        if not self.use_classifier_head and self.param_dict is not None:
            self.param_dict = {k: v for k, v in self.param_dict.items() if k != "fc"}

    def _load_h5py_to_jax(self, h5_file):
        """Recursively load h5py groups/datasets into JAX arrays."""
        result = {}

        def load_recursive(name, obj):
            if isinstance(obj, h5py.Dataset):
                # Convert h5py dataset to JAX array
                result[name] = jnp.array(obj[:])
            elif isinstance(obj, h5py.Group):
                # Recursively load groups
                for key in obj.keys():
                    load_recursive(f"{name}/{key}" if name else key, obj[key])

        for key in h5_file.keys():
            load_recursive(key, h5_file[key])

        # Convert flat dict with '/' separators to nested dict structure
        nested_dict = {}
    ...
    def __call__(self, x, train=True):
        ...
        # Classifier
        x = jnp.mean(x, axis=(1, 2))  # global avg pooling: (B, 7, 7, 512) -> (B, 512)
